    ...
    const loadStudent = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          const id =
            studentData._id != null ? String(studentData._id) : String(studentId);
          const name =
            [studentData.firstName, studentData.lastName].filter(Boolean).join(' ').trim() ||
            studentData.name ||
            'Student';
          let hourlyRate = 15;
          if (typeof studentData.hourlyRate === 'number' && Number.isFinite(studentData.hourlyRate)) {
            hourlyRate = studentData.hourlyRate;
          } else if (studentData.hourlyRateRange) {
